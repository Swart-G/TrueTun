...
class _LinuxShellState extends State<_LinuxShell> {
  ...
  static const _destinations = <NavigationDestination>[
    ...
    NavigationDestination(
        icon: Icon(Icons.memory_outlined), label: 'Processes'),
...
class _TrafficChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final visible =
        samples.length <= 120 ? samples : samples.sublist(samples.length - 120);
...
class _TrafficChartPainter extends CustomPainter {
  ...
  static const _visibleSlots = 120;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final download = _smoothPath(downloadPoints);
    final upload = _smoothPath(uploadPoints);
    canvas.drawPath(
      download,
      Paint()
        ..color = downloadColor
                  ...
                  child: ExpansionTile(
                    ...
                    trailing: PopupMenuButton<String>(
                      ...
                      itemBuilder: (context) => [
                        ...
                        const PopupMenuItem(
                            value: 'test', child: Text('Test all')),
                        const PopupMenuItem(
                    ...
                    children: [
                      ...
                      for (final profile in group.profiles)
                        ListTile(
                          ...
                          onTap: () =>
                              controller.selectProfile(group.id, profile.id),
                        ),
                    ],
                  ),
                );
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          FilledButton.icon(
            onPressed: () async {
              final rule = await showRoutingRuleEditor(
                context,
        ...
        itemBuilder: (context, listIndex) {
              ...
              child: Column(
                children: [
                  SwitchListTile(
                    ...
                    subtitle: Text(
                      data.settings.enabled
                          ? 'Apply the ordered rules below to network traffic'
                  ...
                  ListTile(
                    ...
                    subtitle: const Text(
                      'Action when none of the rules match',
                    ),
          ...
          return Card(
            child: ListTile(
              ...
              subtitle:
                  Text('${_ruleSummary(rule)} → ${_actionLabel(rule.action)}'),
              onTap: () async {
                final updated = await showRoutingRuleEditor(
                  context,
                  platform: RoutingPlatform.linux,
                  existing: rule,
                );
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  child: ListTile(
                    ...
                    title: Text(rule.name),
                    subtitle: Text(
                        '${_processSummary(rule.matcher)} → ${_actionLabel(rule.action)}'),
                    trailing: IconButton(
                      tooltip: 'Delete',
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: const [
                ...
                DropdownMenuItem(
                    value: 'path', child: Text('Exact executable path')),
                DropdownMenuItem(
                    value: 'regex', child: Text('Executable path regex')),
              ],
              onChanged: (value) {
            ...
            DropdownButtonFormField<RouteActionType>(
              ...
              items: const [
                DropdownMenuItem(
                    value: RouteActionType.proxy, child: Text('Proxy')),
                DropdownMenuItem(
                    value: RouteActionType.direct, child: Text('Direct')),
                DropdownMenuItem(
                    value: RouteActionType.block, child: Text('Block')),
              ],
              onChanged: (value) {
...
class _SettingsPage extends ConsumerWidget {
      ...
      body: ListView(
        ...
        children: [
          ...
          ListTile(
            ...
            onTap: () =>
                _editDns(context, controller, preferences, remote: true),
          ),
          ListTile(
            ...
            onTap: () =>
                _editDns(context, controller, preferences, remote: false),
          ),
          ListTile(
  ...
  Future<void> _selectStack(
    ...
    if (value != null) {
      await controller
          .updateCorePreferences(preferences.copyWith(stack: value));
    }
  }
  ...
  Future<void> _selectLogLevel(
    ...
    if (value != null) {
      await controller
          .updateCorePreferences(preferences.copyWith(logLevel: value));
    }
  }
}
...
String _ruleSummary(RoutingRule rule) {
  ...
  final parts = <String>[];
  if (matcher.domains.isNotEmpty)
    parts.add('${matcher.domains.length} domains');
  if (matcher.domainSuffixes.isNotEmpty)
    parts.add('${matcher.domainSuffixes.length} suffixes');
  if (matcher.domainKeywords.isNotEmpty)
    parts.add('${matcher.domainKeywords.length} keywords');
  if (matcher.domainRegexes.isNotEmpty)
    parts.add('${matcher.domainRegexes.length} regexes');
  ...
  if (matcher.processNames.isNotEmpty ||
...
String _processSummary(RuleMatcher matcher) {
  if (matcher.processNames.isNotEmpty)
    return 'name: ${matcher.processNames.join(', ')}';
  if (matcher.processPaths.isNotEmpty)
    return 'path: ${matcher.processPaths.join(', ')}';
